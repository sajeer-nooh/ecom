...
export default function ProductsTable() {
  ...
  const [selectesProduct, setSelectedProduct] = React.useState(null);
  ...
  const filteredItems = React.useMemo(() => {
    ...
    if (statusFilter !== "all" && Array.from(statusFilter).length !== statusOptions.length) {
      filteredProducts = filteredProducts.filter((product) =>
        Array.from(statusFilter).includes(product.category as string),
      );
    }
      ...
      case "price":
        return (
          <div className="flex flex-col">
            <p className="text-bold text-small capitalize">{cellValue} KD</p>
          </div>
        );
      case "stock":
        return (
      ...
      case "actions":
        return (
          <div className="relative flex items-center gap-2">
            ...
            <span onClick={() => deleteProduct(product.id)} className="text-lg text-danger cursor-pointer active:opacity-50">
              <DeleteIcon />
            </span>
          </div>)
      default:
  ...
  return (
    <>
      ...
      <AddEditProductModal isOpen={isOpen} onClose={() => {
        onClose()
        setSelectedProduct(null)
      }} product={selectesProduct} setSelectedProduct={setSelectedProduct}/>
    </>
  );
}
